...
> ⚠️ **Internal only — do not publish to Confluence.** The canonical glossary is [Glossary (14-glossary.md)](14-glossary.md). This file is retained in the repository for reference.
...
|-----------|----------------------|
| **Source hosting** | **GitLab** (`gitlab.digital.homeoffice.gov.uk`) — self-hosted |
...
| **Maven cache paths** | Docker build stages use `/root/.m2/repository`. Drone Maven steps can use `.m2/repository` via `MAVEN_OPTS`. Local developer runs usually use `~/.m2/repository`. |
| **Java version** | Java 17 (Amazon Corretto). Testcontainers requires minimum Java 11; Java 17+ recommended. |
