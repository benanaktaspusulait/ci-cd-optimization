...
| 2 | Dockerfile multi-stage build | Separate dependency resolution → build → runtime. Ship only JRE + JAR (no full JDK, no build tools, no source). Reduces image ~450 MB → ~300 MB. | High | Low | Medium | Medium | Must | CST | 1 | Local-only; no pipeline change needed |
