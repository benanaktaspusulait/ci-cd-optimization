...
| **DoD** | Definition of Done — the shared checklist a task must satisfy before it is considered complete. See [`docs/stories/DEFINITION-OF-DONE.md`](epics/epic-1-pivot-study/stories/DEFINITION-OF-DONE.md). |
...
| **Remote cache** | A BuildKit cache stored in a container registry so that CI runners (which have no persistent local disk) can reuse layers across jobs. See [tech-notes.md](epics/epic-1-pivot-study/stories/tech-notes.md) and post-pilot (remote cache). |
