# T0.4 — Assess Testcontainers feasibility in Drone/DIND
...
## Why
Testcontainers requires Docker daemon access from within the test JVM. The current Drone pipeline provides DIND but it's unclear whether the Maven step can reach it. The ECR pipeline already sets `TESTCONTAINERS_RYUK_DISABLED=true` — this suggests prior exploration but also a known compatibility issue. Without confirming feasibility, Story 3 cannot determine whether Testcontainers will work in CI or only locally.
...
- [ ] Finding informs Story 3 scope (CI or local-only)
